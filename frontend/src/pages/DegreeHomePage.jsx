          ...
          <table className="min-w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 bg-gray-50 text-left text-gray-600">
                ...
                <th className="px-3 py-2 font-semibold">Repeated</th>
              </tr>
            </thead>
            <tbody>
              {section.rows.map((row, idx) => (
                <tr key={`${section.id}-${idx}-${row.course_id}`} className="border-b border-gray-100 hover:bg-gray-50/80">
                  ...
                  <td className="px-3 py-2.5 text-gray-500">{row.repeated || '—'}</td>
                </tr>
              ))}
            </tbody>
          </table>
                ...
                <div>
                  ...
                  {audit && (
                    <p className="mt-2 text-sm text-gray-600">
                      <span className="font-semibold text-gray-800">Credits required:</span> {audit.credits_required}
                      {' · '}
                      <span className="font-semibold text-gray-800">Credits applied:</span> {audit.credits_applied}
                      {' · '}
                  ...
                  {audit?.registration_term_label && (
                    <p className="mt-1 text-xs text-teal-800">
                      Schedule builder uses <strong>{audit.registration_term_label}</strong> mock offerings only.
                    </p>
                  )}
                ...
                <ul className="space-y-3">
                  {(dashboardData.upcoming ?? []).map((sch) => (
                    <li key={sch.id} className="rounded-lg border border-teal-200 bg-teal-50 px-3 py-3 text-sm">
                      ...
                      <p className="text-gray-600 mt-1">Saved in NinerPath</p>
                    </li>
                  ))}
                </ul>
              ) : (
